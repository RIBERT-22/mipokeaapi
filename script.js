...
const fightingButton = document.getElementById("fighting-btn"); // antes fire-btn
const fightingGrid = document.getElementById("fighting-grid");   // antes fire-grid
...
const renderSingle = (pokemon) => {
  ...
  pokemonCard.innerHTML = `
    <div class="card-header">
      <div class="pill pill-number">#${pokemon.id}</div>
...
// --- Render de la grilla de Pokémon Fighting ---
const renderFightingGrid = (list) => {
  if (!list.length) {
    fightingGrid.innerHTML =
      '<p class="placeholder">No se pudieron cargar los Pokémon Fighting.</p>';
    return;
  }

  // Renderiza cada Pokémon Fighting con su tipo principal como clase para colorear la card.
  fightingGrid.innerHTML = list
    .map(
      (pokemon) => `
...
const handleSearch = async (event) => {
  ...
  try {
    ...
    renderSingle(null);
    setStatus("No pudimos encontrar ese Pokémon. Intenta con otro nombre.", "error");
  }
};

// --- Controlador que carga todos los Pokémon de tipo Fighting ---
const handleFighting = async () => {
  fightingButton.disabled = true;
  fightingGrid.innerHTML = '<p class="placeholder">Cargando lista...</p>';
  setStatus("Cargando todos los Pokémon Fighting...", "info");

  try {
    // Paso 1: obtener la lista de Pokémon de tipo Fighting.
    // Endpoint: GET https://pokeapi.co/api/v2/type/fighting -> devuelve nombres/URLs.
    const response = await fetch(`${API_BASE}/type/fighting`);
    if (!response.ok) {
    ...
    const fightingPokemons = detailResults
      .filter((result) => result.status === "fulfilled")
      .map((result) => result.value)
      .sort((a, b) => a.id - b.id);

    renderFightingGrid(fightingPokemons);
    setStatus(`Listo: ${fightingPokemons.length} Pokémon Fighting.`, "ok");
  } catch (error) {
    fightingGrid.innerHTML = '<p class="placeholder">No se pudo consultar la PokéAPI ahora.</p>';
    setStatus("No se pudieron cargar los Pokémon Fighting. Intenta nuevamente.", "error");
  } finally {
    fightingButton.disabled = false;
  }
};
